...
2. Render auto-detects `render.yaml` — click **Apply**.
   ...
   - Start command: `gunicorn --chdir backend main:app --bind 0.0.0.0:$PORT --timeout 120`
3. Deploy. You get a public `*.onrender.com` URL.

## Option B — Docker (Railway / Fly.io / any host)

```bash
docker build -t valo-yt .
docker run -p 8080:8080 valo-yt
```
...
Add a `backend/config.json` with your Valorant API key before the `/api` routes will work.
